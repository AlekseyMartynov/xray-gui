...
static class TapModeOutsideDnsBlock {
    const string SZ_NAME = "xray-gui-outside-dns-block\0";
    ...
    static unsafe void InitEngine() {
        ...
        fixed(HANDLE* engine = &Engine) {
            NativeUtils.MustSucceed(
                PInvoke.FwpmEngineOpen0(default, RPC_C_AUTHN_DEFAULT, default, session, engine)
            );
        }
    }

    static unsafe void InitSubLayer() {
        fixed(char* name = SZ_NAME) {
